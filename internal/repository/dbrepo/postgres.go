...
// SearchAvailabilityForAllRooms returns a slice of avaiable rooms, if any, for given date range
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	if err == bcrypt.ErrMismatchedHashAndPassword {
		return 0, "", errors.New("incorrent password")
	} else if err != nil {
		return 0, "", err
	}
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `
		select 
			...
			rm.id, rm.room_name 
		from reservations r left join rooms rm 
			on (r.room_id = rm.id) 
